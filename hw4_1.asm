...
store macro   di                   ;将数据中的每位数存储在num中
            local L1
      L1:   mov   bx,10            ;把各个位转换为数值，如ax中的81，转换为 8,1存在内存中
...
code segment
      ...
      ;调用store和output子程序输出除以100后的商
            mov    dx,0
            mov    di,6
            store  di                              ;调用store子程序来存储除了最后两位的数字
            mov    cx,10
            add    cx,di                           ;通过di可知输出的数据，以此来确认后续空格的次数
            output di                              ;调用output输出存储在num中的数据
      ...
      ;余数为两位数，将其除以10，分别输出商和余数即可
            ...
            blank  cx                              ;调用blank子程序输出空格，次数由上述cx决定
